...
(function() {
	...
	window.json = { //declared global
		...
		tutorial: {
            "basics": {
                "field 1": "基本规则和技巧。",
                "field 2": "在Phylo中，你的目标是水平移动方砖，让方砖的颜色在垂直方向上尽量的相似。每一个匹配将带来加分。",
                "field 3": "然而，序列并不相同。因此，颜色错配/插空不可避免，它们会带来一些罚分。你的挑战是找到最低代价的方案，在加分与罚分中取得平衡。",
                "field 4": "加分与罚分相对插空而言是次要的！而零散的插空比连续的罚分更多。优先减少插空的数量吧",
                "field 5": "你注意到左边的树了吗？它告诉你需要序列解决的次序。它能让你循序渐渐。获得相同组内保守的模式比增加不同组间的相似性更重要。",
                "field 6": "为了到达最后的挑战，你需要过关斩将。这需要冲击标杆（计算机的分数）。当你的分数达到/超过标杆时，一颗闪闪的星星将在右下角出现。点击它吧，你过关了。当所有的序列都出现的时候，尝试获得更高的分数，点击星星，将提交你的成果。",
