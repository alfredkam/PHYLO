...
FitchCalculator.prototype.getFinalTree = function build(seqs, override){
    ...
    */
    var addToTree = function rec(single, location, out, override){
	// results are stored in out. init "out" if at the first call. 
	    ...
	    alert(location + ":" + single);
            var left  = rec(single[0], location, out[0], override[0]);
            var right = rec(single[1], location, out[1], override[1]);
	    ...
	    //alert(override.value[location] + ":" + left + " + " + right + " = " + out.seq[location] );
            out.ancestor[location] = Math.min(trailingZeroCount(out.seq[location]), 4);
	    //alert(location + ":" + out.seq[location] + "-->" + out.ancestor[location]);
    ...
    for (var i = 0; i < length; i++){
        var result = this.diffLocation(seqs, i);
    ...
    for (var i = 0; i < treearr.length; i++){
        addToTree(treearr[i], i, finaltree, override);
    }
